...
/**
 * Created by changjunkui on 16/5/4.
 * 时间处理类
 */
public class DateUtils {
     ...
     * 日期时间
     */
    public final static String            DEFAULT_PAY_DATE_FORMAT        = "yyyyMMdd";
    ...
    public static boolean defaultDateFormatValidate(String date, boolean lenient) {
        ...
        if(!lenient) {
            try {
                DateUtils.convertStrToDate(String.valueOf(date), DEFAULT_PAY_FORMAT);
                return true;
            } catch (Exception e) {
                return false;
            }
    ...
    public static String addDateOneDay(String datetime) {
    	if (null == datetime) {
    		return datetime;
    	}
    	Date dt1 =  convertStrToDate(datetime,DEFAULT_SPLIT_PAY_FORMAT);
    	Calendar c = Calendar.getInstance();  
        c.setTime(dt1);   //设置当前日期  
        c.add(Calendar.DATE, 1); //日期加1天  
        dt1 = c.getTime();
        return convertDateToStr(dt1,DEFAULT_SPLIT_PAY_FORMAT); 
    }
    public static String addDateTwoDay(String datetime) {
    	if (null == datetime) {
    		return datetime;
    	}
    	Date dt1 =  convertStrToDate(datetime,DEFAULT_SPLIT_PAY_FORMAT);
    	Calendar c = Calendar.getInstance();  
        c.setTime(dt1);   //设置当前日期  
        c.add(Calendar.DATE, 2); //日期加1天  
        dt1 = c.getTime();
        return convertDateToStr(dt1,DEFAULT_SPLIT_PAY_FORMAT); 
    }
    
    public static boolean isOverThreeMonths(String beforeTime,String afterTime) {
    	if (StringUtils.isBlank(beforeTime) || StringUtils.isBlank(afterTime)) {
    		return false;
    	}
    	Date dt2 =  convertStrToDate(afterTime,DEFAULT_SPLIT_PAY_FORMAT);
    	Date dt1 =  convertStrToDate(beforeTime,DEFAULT_SPLIT_PAY_FORMAT);
    	Calendar c = Calendar.getInstance();  
        c.setTime(dt1);   //设置当前日期  
        c.add(Calendar.MONTH, 3); //日期加1天  
        dt1 = c.getTime();
